// Package models cung cấp bảng đăng ký metadata mô hình LLM (cửa sổ ngữ cảnh, giới hạn đầu ra, giá cả),
// nguồn dữ liệu OpenRouter API, baseline biên dịch + làm mới thời gian chạy.
package models
...
type ModelEntry struct {
	Provider            string  `json:"provider"`               // Tên nhà cung cấp đã chuẩn hóa bởi OpenRouter (anthropic/openai/gemini/...)
	ID                  string  `json:"id"`                     // ID mô hình (không chứa tiền tố nhà cung cấp)
...
// ModelRegistry lưu trữ các mô hình đã biết, hỗ trợ giải quyết mờ và hợp nhất thời gian chạy.
...
// DefaultRegistry trả về bảng đăng ký toàn cục (tải lười, an toàn luồng).
// Gọi StartPricingRefresh ở giai đoạn khởi động để làm mới giá/thông tin cửa sổ ở nền.
...
// Resolve tra cứu mục theo định danh mô hình (có thể là "provider/model", ID đầy đủ, hoặc tên một phần).
//
// Thứ tự khớp:
...
//
// Khi trúng nhiều, ưu tiên trả về bí danh không chứa hậu tố ngày (ví dụ claude-sonnet-4 ưu tiên hơn claude-sonnet-4-20250514).
func (r *ModelRegistry) Resolve(pattern string) (*ModelEntry, bool) {
	...
	if idx := strings.Index(pattern, "/"); idx > 0 {
		...
		// tra lại chỉ dùng modelID, đảm bảo "google/gemini-2.5-pro" tìm được mục gemini.
...
// ResolveContextWindow trả về cửa sổ ngữ cảnh của một mô hình; trả về 0 khi không tìm thấy.
...
// List trả về tất cả mô hình (filter tùy chọn, chuỗi rỗng nghĩa là tất cả).
...
// MergeModels hợp nhất theo provider+id không phân biệt chữ hoa/thường.
// Giá/cửa sổ/MaxTokens/Name khác không sẽ ghi đè mục hiện có; mục mới thêm trực tiếp vào cuối.

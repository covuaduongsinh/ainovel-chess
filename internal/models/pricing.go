...
const (
	...
	// maxModelAgeDays giữ nhất quán với gen_models.go: mô hình vượt quá tuổi này được coi là lỗi thời, loại bỏ.
	maxModelAgeDays = 730
)

// providerMap chuẩn hóa tiền tố vendor của OpenRouter thành tên provider nội bộ.
// Nhà cung cấp chưa liệt kê sẽ bị bỏ qua, tránh lấy về các mục không dùng được.
...
// StartPricingRefresh khởi goroutine nền để làm mới dữ liệu mô hình.
// Đọc cache trên đĩa trước (TTL 24h), nếu hết hạn hoặc không tồn tại thì lấy dữ liệu mới và lưu xuống đĩa.
// Khi cacheDir rỗng thì bỏ qua cache đĩa, vẫn thử lấy qua mạng.
func StartPricingRefresh(registry *ModelRegistry, cacheDir string) {
	go func() {
		...
		if models == nil {
			...
			if err != nil {
				slog.Warn("Làm mới metadata mô hình thất bại", "module", "models", "err", err)
				return
			}
			models = fetched
		...
		slog.Info("Metadata mô hình đã sẵn sàng", "module", "models", "count", len(models))
	}()
}
...
// isStaleModel lọc mô hình lỗi thời theo maxModelAgeDays.
// Giá trị 0 hoặc âm được coi là thiếu dữ liệu, xử lý như "mô hình cũ" và loại bỏ trực tiếp.
...
// tokenToMillion chuyển đổi "giá USD mỗi token" trả về từ OpenRouter thành "giá USD mỗi 1M token".

...
// completedBook xay dung mot cuon sach da hoan ket N chuong (phase=complete, CompletedChapters=1..n).
...
func TestReopenBookReopensCompletedBook(t *testing.T) {
	...
	args, _ := json.Marshal(map[string]any{"chapters": []int{3, 1}, "reason": "Lam sach ky tu dac biet"})
	...
	if len(p.PendingRewrites) != 2 || p.PendingRewrites[0] != 3 || p.PendingRewrites[1] != 1 {
		t.Errorf("PendingRewrites = %v, want [3 1] (giu nguyen thu tu vao hang doi)", p.PendingRewrites)
	}
...
func TestReopenBookRejectsNonCompleteBook(t *testing.T) {
	// Sach dang viet (chua hoan ket) khong the reopen
	s := store.NewStore(t.TempDir())
	if err := s.Init(); err != nil {
		t.Fatalf("Init: %v", err)
	}

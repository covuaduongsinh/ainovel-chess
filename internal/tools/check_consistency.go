...
// CheckConsistencyTool trả về nội dung chương và toàn bộ dữ liệu trạng thái, cho Agent tự đối chiếu.
// Công cụ IO thuần túy: chỉ chịu trách nhiệm tải dữ liệu, không tiêm chỉ lệnh.
...
func (t *CheckConsistencyTool) Description() string {
	return "Tải bản nháp đã viết và dữ liệu đối chiếu (quy tắc thế giới, phục bút, quan hệ, bí danh, tóm tắt gần đây) để bạn kiểm tra tính nhất quán. Phải gọi sau draft_chapter"
}
func (t *CheckConsistencyTool) Label() string { return "Kiểm tra nhất quán" }

// Công cụ chỉ đọc (chỉ bổ sung sự kiện checkpoint, không thay đổi trạng thái), có thể được lên lịch song song.
...
func (t *CheckConsistencyTool) Execute(_ context.Context, args json.RawMessage) (json.RawMessage, error) {
	...
	// dữ liệu đối chiếu: giữ lại dữ liệu kiểm tra nhất quán toàn cục, tránh tải lại dữ liệu cửa sổ đã có trong novel_context

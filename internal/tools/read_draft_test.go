...
func TestReadChapterFinal(t *testing.T) {
	...
	if err := store.Drafts.SaveFinalChapter(1, "Ban thao cuoi cung chuong mot."); err != nil {
...
func TestReadChapterDraft(t *testing.T) {
	...
	if err := store.Drafts.SaveDraft(3, "Noi dung ban nhap chuong ba."); err != nil {
...
func TestReadChapterDialogue(t *testing.T) {
	...
	if err := store.Characters.Save([]domain.Character{
		{Name: "Truong Tam", Aliases: []string{"Lao Truong"}},
	}); err != nil {
		t.Fatalf("SaveCharacters: %v", err)
	}
	if err := store.Drafts.SaveFinalChapter(1, "Truong Tam dung day. \"Toi khong dong y voi ke hoach nay,\" Truong Tam noi lanh lung."); err != nil {
		t.Fatalf("SaveFinalChapter: %v", err)
	}

	tool := NewReadChapterTool(store)
	args, _ := json.Marshal(map[string]any{"source": "final", "character": "Truong Tam"})
	result, err := tool.Execute(context.Background(), args)
	if err != nil {
		t.Fatalf("Execute: %v", err)
	}
	...
	if payload.Character != "Truong Tam" {
		t.Fatalf("expected character Truong Tam, got %s", payload.Character)
	}
}
...
func TestReadChapterRange(t *testing.T) {
	...
	for i := 1; i <= 3; i++ {
		if err := store.Drafts.SaveFinalChapter(i, "Day la mot doan noi dung chinh."); err != nil {
...
func TestDraftChapterWrite(t *testing.T) {
	...
	args, _ := json.Marshal(map[string]any{
		"chapter": 1,
		"content": "Day la noi dung toan bo chuong, viet xong mot lan.",
		"mode":    "write",
	})
	result, err := tool.Execute(context.Background(), args)
	if err != nil {
...
func TestDraftChapterAppend(t *testing.T) {
	...
	if err := store.Drafts.SaveDraft(2, "Phan dau."); err != nil {
		t.Fatalf("SaveDraft: %v", err)
	}
...
func TestPlanChapterMarksInProgress(t *testing.T) {
	...
	args, _ := json.Marshal(map[string]any{
		"chapter":  1,
		"title":    "Khoi dau",
		"goal":     "Thiet lap tinh huong",
		"conflict": "Mon no den gan",
		"hook":     "Phat hien manh moi",
	})
...
func TestDraftChapterRejectsCompleted(t *testing.T) {
	...
	_ = s.Drafts.SaveDraft(1, "Noi dung chinh chuong mot")
	...
	args, _ := json.Marshal(map[string]any{
		"chapter": 1,
		"content": "Thu ghi de len chuong da nop",
	})
	result, err := tool.Execute(context.Background(), args)
	if err != nil {

...
// SaveFoundationTool lưu thiết lập nền tảng (premise/outline/characters), chỉ dành cho Architect.
...
func (t *SaveFoundationTool) Description() string {
	return "Lưu thiết lập nền tảng tiểu thuyết (premise/outline/characters/world_rules/compass v.v.). **Đây là điểm vào bền vững duy nhất**: nội dung không được lưu qua công cụ này sẽ không vào store, chỉ xuất Markdown/JSON trong tin nhắn đồng nghĩa với mất dữ liệu. Tham số cố định là {type, content, scale?, volume?, arc?}. type có thể là premise / outline / layered_outline / characters / world_rules / expand_arc / append_volume / update_compass / complete_book. Khi type là premise, content phải là chuỗi Markdown; các loại khác content ưu tiên truyền trực tiếp mảng hoặc đối tượng JSON. expand_arc mở rộng các chương chi tiết của cung khung xương (cần volume + arc); append_volume thêm tập mới (content là VolumeOutline JSON đầy đủ, bao gồm cấu trúc cung); update_compass cập nhật hướng kết thúc (content là StoryCompass JSON); complete_book tuyên bố hoàn thành toàn bộ cuốn sách (content truyền đối tượng rỗng {}, đẩy trực tiếp Phase=Complete; phải vượt qua danh sách kiểm tra kết thúc tập trước khi gọi, và không có hàng đợi trả lại). scale là tùy chọn, chỉ cho phép short / mid / long."
}
func (t *SaveFoundationTool) Label() string { return "Lưu thiết lập" }

// Công cụ ghi (cập nhật đa miền Outline/Progress/Characters), cấm song song.
...
func (t *SaveFoundationTool) Schema() map[string]any {
	return schema.Object(
		...
		schema.Property("content", map[string]any{
			"description": "Nội dung. premise truyền chuỗi Markdown; các loại khác truyền trực tiếp mảng hoặc đối tượng JSON, cũng tương thích truyền chuỗi JSON. Khi expand_arc truyền mảng chương.",
		}).Required(),
		schema.Property("scale", schema.Enum("cấp độ hoạch định", "short", "mid", "long")),
...
func (t *SaveFoundationTool) Execute(_ context.Context, args json.RawMessage) (json.RawMessage, error) {
	...
	// Giai đoạn viết cấm phủ toàn bộ dàn ý, chỉ cho phép thao tác tăng dần (expand_arc / append_volume)
	if (a.Type == "outline" || a.Type == "layered_outline") && t.isWriting() {
		return nil, fmt.Errorf(
			"giai đoạn viết cấm dùng %s để phủ toàn bộ dàn ý. Hãy dùng expand_arc mở rộng cung khung xương, hoặc append_volume thêm tập mới: %w", a.Type, errs.ErrToolPrecondition)
	}
	...
	case "complete_book":
		// Điểm vào duy nhất để hoàn thành toàn bộ cuốn sách: đẩy trực tiếp Phase=Complete.
		// Chỉ cho phép ở giai đoạn Writing, ngăn giai đoạn hoạch định gọi nhầm bỏ qua toàn bộ quá trình viết.
		// Từ chối gọi khi có hàng đợi trả lại — đảm bảo PendingRewrites chạy xong mới được kết thúc.
		...
		if len(progress.PendingRewrites) > 0 {
			return nil, fmt.Errorf("còn %d chương trong hàng đợi trả lại, xử lý xong rồi gọi complete_book: %w", len(progress.PendingRewrites), errs.ErrToolPrecondition)
		}
	...
	case "update_compass":
		...
		// Lớp công cụ bắt buộc ghi đè LastUpdated bằng số chương đã hoàn thành hiện tại, không tin LLM tự điền.
		// LLM thường quên điền hoặc để 0, sẽ khiến diag.CompassDrift báo nhầm, Router định tuyến sai.
	...
	// Trả về các mục chưa hoàn thành còn lại, hướng dẫn Architect tiếp tục hoặc kết thúc;
	// khi đủ, đẩy phase lên writing một lần, tránh Coordinator quay lại phân công thêm.
...
// và gợi ý sửa phổ biến nhất, để LLM lần thử tiếp theo có thể định vị trực tiếp thay vì đoán mò.
func decodeFoundationJSON(typeName, content string, out any) error {
	err := json.Unmarshal([]byte(content), out)
	if err == nil {
		return nil
	}
	hint := `Nguyên nhân thường gặp: dấu nháy kép trong giá trị chuỗi chưa được escape thành \", xuống dòng chưa escape thành \n, hoặc thiếu dấu phẩy giữa các trường đối tượng. Vui lòng tạo lại toàn bộ đoạn một lần nữa.`

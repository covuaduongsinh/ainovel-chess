...
// PlanChapterTool lưu ý tưởng chương, Agent tự quyết độ hạt hoạch định.
...
func (t *PlanChapterTool) Description() string {
	return "Lưu ý tưởng viết chương. Agent tự quyết độ hạt hoạch định, không bắt buộc tách cảnh"
}
func (t *PlanChapterTool) Label() string { return "Hoạch định chương" }
...
func (t *PlanChapterTool) Schema() map[string]any {
	return schema.Object(
		...
		schema.Property("goal", schema.String("mục tiêu chương này")).Required(),
		...
		schema.Property("emotion_arc", schema.String("đường cung cảm xúc")),
		schema.Property("notes", schema.String("ghi chú tự do (bất cứ điều gì bạn cần nhớ khi viết)")),
		schema.Property("required_beats", schema.Array("các điểm đẩy tiến bắt buộc hoàn thành trong chương này", schema.String(""))),
		schema.Property("forbidden_moves", schema.Array("các diễn biến rõ ràng không được xảy ra trong chương này", schema.String(""))),
		schema.Property("continuity_checks", schema.Array("các điểm liên tục cần đặc biệt đối chiếu trong chương này", schema.String(""))),
		schema.Property("evaluation_focus", schema.Array("các hạng mục Editor cần kiểm tra trọng tâm", schema.String(""))),
		schema.Property("emotion_target", schema.String("tùy chọn: cảm xúc chính mà chương này muốn độc giả trải nghiệm")),
		schema.Property("payoff_points", schema.Array("tùy chọn: điểm tình tiết hoặc điểm thu hồi mà chương trọng tâm muốn hồi đáp", schema.String(""))),
		schema.Property("hook_goal", schema.String("tùy chọn: ham muốn đọc tiếp hoặc mục tiêu ẩn số mà cuối chương muốn thúc đẩy")),
	)
}
...
func (t *PlanChapterTool) Execute(_ context.Context, args json.RawMessage) (json.RawMessage, error) {
	...
	if t.store.Progress.IsChapterCompleted(plan.Chapter) {
		return json.Marshal(map[string]any{
			...
			"skipped":   true,
			"completed": true,
			"reason":    fmt.Sprintf("chương %d đã giao hoàn thành, không thể hoạch định lại", plan.Chapter),
		})
	}
	...
	return json.Marshal(map[string]any{
		"planned":   true,
		"chapter":   plan.Chapter,
		"next_step": "Ngay lập tức gọi draft_chapter(chapter=số chương này, content=chuỗi chính văn đầy đủ) để viết chính văn, không hoạch định lại cùng một chương",
	})
}

...
func TestPayoffMissPattern(t *testing.T) {
	snap := &Snapshot{
		Plans: map[int]*domain.ChapterPlan{
			1: {Chapter: 1, Contract: domain.ChapterContract{PayoffPoints: []string{"thang-tran-dau-tien"}}},
			2: {Chapter: 2, Contract: domain.ChapterContract{PayoffPoints: []string{"xac-nhan-quan-he-dong-doi"}}},
			3: {Chapter: 3, Contract: domain.ChapterContract{PayoffPoints: []string{"he-lo-mot-goc-su-that"}}},
		},
		Reviews: map[int]*domain.ReviewEntry{

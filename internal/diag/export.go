...
// ExportRelPath là vị trí cố định của file chẩn đoán đã khử nhận dạng tương đối với thư mục output (một file ghi đè).
const ExportRelPath = "meta/diag-export.md"

// Export chẩn đoán đầy đủ + render + ghi đĩa, trả về đường dẫn tuyệt đối đã ghi. Dùng cho headless / gọi bên ngoài.
func Export(s *store.Store) (string, error) {
	rep, rc := Diagnose(s)
	return WriteExport(s, rep, rc)
}

// WriteExport render và ghi đĩa Report + RuntimeCapture đã tính sẵn, không thu thập lại.
// Dùng cho lệnh /diag tái sử dụng kết quả của Diagnose.
...
// RenderExport kết hợp Report sáng tác + RuntimeCapture thành Markdown đã khử nhận dạng.
func RenderExport(rep Report, rc RuntimeCapture) []byte {
	...
	b.WriteString("> ⚠️ Đã khử nhận dạng: nội dung tiểu thuyết / prompt / suy nghĩ đã bị xóa, chỉ giữ lại khung hành vi. Có thể dán thẳng vào issue.\n\n")
	...
	// 2. Phát hiện chẩn đoán (chỉ runtime; chẩn đoán sáng tác có chứa cốt truyện/phục bút, để báo cáo trên màn /diag, không đưa vào export có thể chia sẻ)
	...
	if len(rc.Repeats) > 0 {
		b.WriteString("- Chữ ký tần suất cao (cửa sổ gần ≥3 lần, bao gồm tool lặp bình thường, chỉ để tham khảo):\n")
	...
	if len(rc.DupContent) > 0 {
		b.WriteString("- Tạo lặp cùng đoạn văn bản (cùng sha):\n")
	...
	// 5. Kiểm tra tự khử nhận dạng
	b.WriteString("\n## 5. Kiểm tra khử nhận dạng\n\n")
	fmt.Fprintf(&b, "- Đã khử %d khối văn bản · nội dung lọt ra 0 chỗ\n", rc.RedactedTexts)
	if len(rc.Sources) > 0 {
...
// formatSkel render một sự kiện khung thành một dòng đơn, theo thứ tự dispatch.

...
// StaleForeshadow phát hiện phục bút lâu dài chưa được thúc đẩy.
func StaleForeshadow(snap *Snapshot) []Finding {
	...
	return []Finding{{
		...
		Title:      fmt.Sprintf("Phục bút đình trệ: %d mục vượt quá %d chương chưa được thúc đẩy", len(stale), threshold),
		Evidence:   strings.Join(stale, "; "),
		Suggestion: "Tải nhắc phục bút của novel_context có thể chưa hoạt động, hoặc prompt Writer thiếu hướng dẫn thúc đẩy phục bút. Kiểm tra foreshadow_ledger và logic tiêm ngữ cảnh.",
	}}
}

// CompassDrift phát hiện la bàn lâu dài chưa được cập nhật.
func CompassDrift(snap *Snapshot) []Finding {
	...
	if snap.Compass == nil {
		if snap.CompletedCount() > 5 {
			return []Finding{{
				...
				Title:      "Chế độ tiểu thuyết dài thiếu la bàn",
				Evidence:   fmt.Sprintf("layered=true, completed=%d, compass=nil", snap.CompletedCount()),
				Suggestion: "Architect nên tạo compass trong khi lập kế hoạch ban đầu. Kiểm tra architect-long.md có chứa lệnh tạo compass không.",
			}}
		}
		return nil
	}
	...
	return []Finding{{
		...
		AutoLevel:  AutoNone,
		Target:     "prompt.architect",
		Title:      fmt.Sprintf("La bàn chưa được cập nhật trong %d chương", gap),
		...
		Suggestion: "Architect nên cập nhật compass tại ranh giới cung/tập. Kiểm tra architect-long.md có chứa lệnh cập nhật compass không.",
	}}
}

// OutlineExhausted phát hiện dàn ý đã cạn kiệt nhưng tiểu thuyết chưa hoàn thành.
func OutlineExhausted(snap *Snapshot) []Finding {
	...
	return []Finding{{
		...
		Title:      fmt.Sprintf("Dàn ý cạn kiệt: đã hoàn thành %d chương >= đã lập kế hoạch %d chương", completed, outlinedCount),
		Evidence:   fmt.Sprintf("phase=%s, completed=%d, outlined=%d", p.Phase, completed, outlinedCount),
		Suggestion: "Tín hiệu mở rộng/tập mới có thể chưa kích hoạt. Kiểm tra chiến lược commit phía host và logic khôi phục, xác nhận phát hiện ranh giới cung, expand_arc hoặc append_volume có thực thi bình thường không.",
	}}
}

// MissingSummaries phát hiện các chương đã hoàn thành thiếu tóm tắt.
func MissingSummaries(snap *Snapshot) []Finding {
	...
	return []Finding{{
		...
		Suggestion: "Tóm tắt là chìa khóa cho tính liên tục ngữ cảnh. Kiểm tra logic ghi tóm tắt của commit_chapter có hoạt động bình thường không.",
	}}
}

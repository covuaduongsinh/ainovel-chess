...
func TestCheckpointStore_EmptyDigestNotIdempotent(t *testing.T) {
	...
	// digest rỗng không tham gia vào loại trừ bất biến
...
func TestCheckpointStore_Reset(t *testing.T) {
	...
	// Sau Reset seq được đặt lại: lần nối tiếp theo bắt đầu từ 1
...
func TestCheckpointStore_RestoreFromDisk(t *testing.T) {
	...
	// Mô phỏng khởi động lại: phiên bản mới tải từ cùng thư mục
	io2 := newIO(dir)
	...
	// seq nên tiếp nối từ 4, và tính bất biến vẫn có hiệu lực
...
func TestCheckpointStore_SeqNotConsumedOnWriteFailure(t *testing.T) {
	...
	// Đặt tệp jsonl thành chỉ đọc để làm cho lần ghi OpenFile tiếp theo thất bại

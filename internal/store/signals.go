...
// SignalStore quản lý các tệp tín hiệu một lần (kết quả commit/review, trạng thái chờ khôi phục).
...
// LoadAndClearLastCommit nguyên tử đọc và xóa tín hiệu commit, ngăn chặn điều kiện tranh chấp TOCTOU.
...
// SavePendingCommit lưu trạng thái nộp chương chờ khôi phục.
...
// LoadPendingCommit đọc trạng thái nộp chương chờ khôi phục.
...
// ClearLastReview xóa tệp tín hiệu thẩm định.
func (s *SignalStore) ClearLastReview() error {
	return s.io.RemoveFile("meta/last_review.json")
}

// LoadAndClearLastReview nguyên tử đọc và xóa tín hiệu thẩm định.
...
// ClearStaleSignals dọn dẹp các tệp tín hiệu còn sót lại (gọi khi khởi động lại tiến trình).

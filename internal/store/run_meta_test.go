...
func TestAppendSteerEntry(t *testing.T) {
	...
	// Lần thêm đầu tiên (meta/run.json chưa tồn tại)
	e1 := domain.SteerEntry{Input: "Đổi nhân vật chính thành nữ", Timestamp: "2026-03-07T10:01:00+08:00"}
	...
	if meta.SteerHistory[0].Input != "Đổi nhân vật chính thành nữ" {
		t.Errorf("input mismatch: %s", meta.SteerHistory[0].Input)
	}

	// Thêm mục thứ hai
	e2 := domain.SteerEntry{Input: "Thêm tình tiết đảo ngược", Timestamp: "2026-03-07T10:02:00+08:00"}
...
func TestInitRunMeta_PreservesHistory(t *testing.T) {
	...
	// Tạo RunMeta có lịch sử trước
	_ = store.RunMeta.Save(domain.RunMeta{
		StartedAt:    "old",
		Provider:     "openai",
		Style:        "fantasy",
		Model:        "old-model",
		SteerHistory: []domain.SteerEntry{{Input: "Can thiệp lịch sử", Timestamp: "ts"}},
	...
	if len(meta.SteerHistory) != 1 || meta.SteerHistory[0].Input != "Can thiệp lịch sử" {

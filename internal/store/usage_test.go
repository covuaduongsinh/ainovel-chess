...
// TestUsageStore_LoadMissing xac nhan khi tep khong ton tai tra ve (nil, nil), de ben goi di theo replay.
...
// TestUsageStore_RoundTrip ghi roi doc lai, xac nhan du lieu tich luy tro ve nguyen ven.
...
// TestUsageStore_LoadSchemaMismatch xac nhan khi nang cap schema trong tuong lai tep cu bi bo di
// (de host di theo replay de xay dung lai), khong nhet nham cac truong khong tuong thich vao tracker.
func TestUsageStore_LoadSchemaMismatch(t *testing.T) {
	dir := t.TempDir()
	us := NewUsageStore(newIO(dir))

	// Tu viet mot bo du lieu cu voi schema=0

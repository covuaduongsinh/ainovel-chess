...
// GetChapterOutline lấy mục dàn ý của chương được chỉ định.
...
// ClearLayeredOutline xóa các tệp dàn ý phân tầng.
...
// GetChapterFromLayered tìm kiếm trong dàn ý phân tầng theo số chương toàn cục.
...
// LocateChapter xác định tập và cung chứa chương theo số chương toàn cục.
...
// HasNextArc kiểm tra có còn cung tiếp theo không.
func (b *ArcBoundary) HasNextArc() bool {
	return b.NextVolume > 0 || b.NextArc > 0
}

// CheckArcBoundary kiểm tra một chương có phải là chương cuối của cung/tập không.
func (s *OutlineStore) CheckArcBoundary(chapter int) (*ArcBoundary, error) {
	...
	// Next*/NeedsExpansion/NeedsNewVolume chỉ có ý nghĩa ở cuối cung, nếu không sẽ khiến coordinator lầm tưởng cần mở rộng cung tiếp theo sớm.
	if !isLastChInArc {
		return b, nil
	}
...
// expandArcUnlocked phương thức nội bộ, được gọi trong phối hợp liên miền Store.ExpandArc.
...
// appendVolumeUnlocked phương thức nội bộ, được gọi trong phối hợp liên miền Store.AppendVolume.
...
// SaveCompass lưu la bàn hướng kết thúc.
...
// LoadCompass đọc la bàn hướng kết thúc.

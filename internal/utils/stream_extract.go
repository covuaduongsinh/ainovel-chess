...
// JSONFieldExtractor trích xuất giá trị chuỗi của một trường chỉ định từ các mảnh JSON luồng.
//
// Khi LLM sinh tool call theo luồng, tham số đến từng mảnh một (OpenAI/Anthropic)
// hoặc đến một lần duy nhất (Gemini). Bộ trích xuất này dùng máy trạng thái quét từng ký tự,
// phát hiện key mục tiêu rồi trích xuất giá trị chuỗi, xử lý escape JSON.
...
// ThinkingSep là ký hiệu phân cách giữa văn bản suy nghĩ và nội dung chính.
// StreamFilter chèn ký hiệu này trước đoạn văn bản suy nghĩ, TUI dựa vào đó để chuyển kiểu render.
const ThinkingSep = "\x02"

// StreamFilter phân biệt phản hồi văn bản và tool call JSON của SubAgent.
// Phản hồi văn bản được đánh dấu là nội dung suy nghĩ (tiền tố ThinkingSep); tool call JSON chỉ trích xuất trường chỉ định.
//
// Cách xác định: gặp { thì vào chế độ JSON (theo dõi độ sâu dấu ngoặc nhọn),
...
type StreamFilter struct {
	...
	escJSON    bool // ký tự escape trong chuỗi JSON
...
// Phản hồi văn bản được xuất trực tiếp; giá trị trường mục tiêu trong JSON được trích xuất và xuất; phần còn lại của cấu trúc JSON bị loại bỏ.

...
func (s *simulationState) refresh(contentW int) {
	...
	case s.err != nil:
		b.WriteString(errStyle.Render("Xử lý chân dung mô phỏng thất bại"))
		b.WriteString("\n")
		b.WriteString(dimStyle.Render("Esc đóng panel"))
	default:
		b.WriteString(okStyle.Render("Chân dung mô phỏng đã sẵn sàng, Agent tiếp theo sẽ đọc từ novel_context"))
		b.WriteString("\n")
		b.WriteString(dimStyle.Render("Esc đóng panel"))
	}
...
func renderSimulationModal(width, height int, s *simulationState) string {
	...
	modal := renderPaddedModalFrame(boxW, boxH, "Chân dung mô phỏng", hint, strings.Split(s.viewport.View(), "\n"))
...
func startImportSimulation(rt *host.Host, reqID int, args []string, width, height int) (*simulationState, tea.Cmd, error) {
	...
	state := newSimulationState(reqID, "Nhập chân dung mô phỏng", args[0], width, height, cancel)
	return state, listenSimulationEvent(reqID, ch), nil
}

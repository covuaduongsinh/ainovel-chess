...
func (m startupMode) subtitle() string {
	switch m {
	case startupModeCoCreate:
		return "Trò chuyện với AI trước, rồi mới bắt đầu sáng tác"
	default:
...
func placeholderForNewMode(mode startupMode) string {
	switch mode {
	case startupModeCoCreate:
		return "Nhập ý tưởng cốt lõi, Enter để bắt đầu cộng tác với AI"
	default:
		return "Nhập yêu cầu tiểu thuyết, Enter để bắt đầu sáng tác ngay"
	}
}
...
type cocreateState struct {
	...
	stage      bool // true=cộng tác theo giai đoạn (lập kế hoạch trong khi chạy); false=cộng tác khởi động lạnh (làm rõ yêu cầu trước khi khởi động)
	awaiting   bool
	reqID      int
	...
	convFollow bool // true: nội dung stream mới tự động cuộn xuống đáy; khi người dùng cuộn lên đặt false để dừng theo dõi
	// focusPrompt xác định ↑↓/PgUp/PgDn/Home/End cuộn cột nào: false=cột hội thoại bên trái (mặc định),
	// true=cột lệnh sáng tác bên phải. Trang chào đã tắt báo cáo chuột (giữ tính năng sao chép gốc), cột phải tràn thì dùng Tab chuyển tiêu điểm rồi cuộn bàn phím.
	focusPrompt bool
}
...
// stageCoCreateOpener là câu mở đầu tổng hợp cho cộng tác theo giai đoạn, gửi như lượt user kickoff đến LLM,
// để trợ lý chủ động mở đầu dựa trên "trạng thái câu chuyện hiện tại", thay vì ngồi chờ người dùng nói trước.
const stageCoCreateOpener = "Để tôi tạm dừng một chút, muốn cùng bạn lập kế hoạch cho hướng đi tiếp theo."

// stageCoCreateSystemLine là cách hiển thị trung tính của câu mở đầu trong UI: câu mở đầu về bản chất là do hệ thống tổng hợp,
// người dùng chưa thực sự gõ vào, nên không giả vờ là lời nói của "bạn", thay bằng dòng hệ thống để giải thích ngữ cảnh
// (nó vẫn được gửi đến LLM dưới dạng stageCoCreateOpener, xem xử lý đặc biệt i==0 trong renderCoCreateConversationPanel).
const stageCoCreateSystemLine = "Đã tạm dừng sáng tác, vào giai đoạn cộng tác —— AI sẽ kết hợp tiến độ câu chuyện hiện tại cùng bạn lập kế hoạch hướng đi tiếp theo."

// newStageCoCreateState tạo trạng thái cộng tác theo giai đoạn: seed câu mở đầu và đánh dấu stage, để runCoCreate đi theo
...
func renderStartupModeBar(width int, mode startupMode) string {
	...
	cocreate := renderStartupModePill(mode == startupModeCoCreate, "Cộng tác lập kế hoạch")
...
// Cột trái chứa hội thoại và ô nhập liệu (xếp chồng lên dưới), cột phải chứa bản thảo lệnh sáng tác; tổng bằng chiều rộng nội dung modal.
...
func renderCoCreateBody(width, height int, state *cocreateState, errMsg, inputView string, spinnerFrame int) string {
	...
	// Border phải được vẽ bởi container leftCol bên ngoài, xuyên suốt từ đỉnh đến đáy body;
	// conversation / suggestions / input đều không tự vẽ border phải của mình.
	// Input vẫn là khung bo tròn hoàn chỉnh, margin trái phải mỗi cột 1 căn chỉnh với padding của conversation,
...
// Các thẻ khác (<draft>/<ready>/<suggestions>) là trường giao thức dành cho mô hình ở vòng tiếp theo, không nên hiển thị trực tiếp cho người dùng.
// Khi mô hình tuân thủ một nửa (bỏ sót thẻ mở <reply>), từ đầu đến </reply> hoặc thẻ mở tiếp theo đều tính là reply.
// Khi hoàn toàn không có thẻ nào (đường dẫn giảm cấp) trả về nguyên vẹn.
...
// renderCoCreateSuggestions hiển thị dòng gợi ý AI phía trên input. Khi đang chờ hoặc không có gợi ý
// trả về chuỗi rỗng, để layout tự xẹp lại không để trống. Tối đa 3 gợi ý, chọn bằng phím số 1/2/3.
func renderCoCreateSuggestions(width int, state *cocreateState) string {
	...
	lines := []string{hintStyle.Render("Gợi ý AI (nhấn phím số để điền vào ô nhập liệu):")}
	for i, s := range sugs {
...
// coCreateInputWidth tính chiều rộng ký tự thực tế có thể nhập vào textarea.
// Trang trí cột trái: đường dọc phải ngoài 1 + margin trái phải input 2 + border 2 + padding 2 = 7 cột;
// textarea tự chiếm 2 cột prompt+cursor; vậy textareaW = leftW - 9.
...
func renderCoCreateModal(width, height int, state *cocreateState, errMsg, inputView string, spinnerFrame int, quitPending bool) string {
	...
	// title / subtitle / hint đặt bên ngoài modal (căn giữa phía trên và dưới), để phần trong modal
	// hoàn toàn dành cho body —— đường dọc phải cột trái và cột phải xuyên từ đỉnh đến đáy modal.
	...
	// Do đó giảm boxH đi 5 dòng cho trang trí bên ngoài modal, tránh tràn terminal.
	contentH := boxH - 5
	if contentH < 10 {
		contentH = 10
	}

	titleText, subtitleText := "Cộng tác lập kế hoạch", "Làm rõ yêu cầu trước, rồi mới bắt đầu sáng tác"
	if state.stage {
		titleText, subtitleText = "Cộng tác giai đoạn", "Lập kế hoạch hướng đi tiếp theo, rồi tiếp tục sáng tác"
	}
	...
	if quitPending {
		// quitPending đồng bộ với inputHints(); nếu không modal cộng tác sẽ che thanh đáy, người dùng không thấy "nhấn Ctrl+C một lần nữa".
...
func coCreateHint(state *cocreateState) string {
	...
	case state.awaiting:
		return "AI đang trả lời · ↑↓ cuộn hội thoại · con lăn cuộn lệnh · Esc thoát"
	case state.canStart():
		action := "Ctrl+S bắt đầu sáng tác"
		if state.stage {
			action = "Ctrl+S áp dụng và tiếp tục"
		}
		return "Enter tiếp tục bổ sung · " + action + " · ↑↓ cuộn hội thoại · con lăn cuộn lệnh · Esc thoát"
	default:
		return "Enter gửi · ↑↓ cuộn hội thoại · con lăn cuộn lệnh · Esc thoát"
	}
}

func renderCoCreateConversationPanel(width, height int, state *cocreateState, errMsg string, spinnerFrame int) string {
	// Không tự vẽ border —— đường dọc phải do container leftCol bên ngoài vẽ thống nhất.
	// Tổng chiều rộng cột = width; style.Width = contentW = width-2; sau Padding(0,1) vùng nội dung = contentW-2.
	// Trong dòng còn phải trừ tiền tố 2 cột kiểu "▌ " / "  ", nếu không sau wrap mỗi dòng + tiền tố sẽ tràn 2 cột,
	// gây terminal ngắt dòng vật lý —— lipgloss vẫn cho rằng chiều cao modal cố định, nhưng terminal thực tế cao hơn,
	// khi thinking stream liên tục kích hoạt sẽ biểu hiện là "chiều cao khung ngoài giật".
	// Vì vậy wrapW = contentW - 4.
	contentW := width - 2
	if contentW < 12 {
		contentW = 12
	}
	...
	for i, item := range state.session.History() {
		...
		// Câu mở đầu tổng hợp của cộng tác giai đoạn (luôn là tin nhắn user của history[0]) hiển thị dưới dạng dòng hệ thống trung tính,
		// không giả vờ là đầu vào của người dùng; nó vẫn được gửi đến LLM như lượt user kickoff.
		...
		if isUser {
			...
			// history lưu trữ Raw đầy đủ bốn đoạn của assistant (dùng cho ngữ cảnh mô hình), UI chỉ hiển thị đoạn [REPLY].
	...
	if state.awaiting {
		...
		// sparkle trang trí: để người dùng luôn thấy "AI đang làm việc"
	...
	// Chiều cao vp = chiều cao panel - 1 dòng tiêu đề. Sau SetContent nếu người dùng đang ở đáy,
	// tự động cuộn xuống mới nhất (theo dõi stream); sau khi người dùng cuộn lên, convFollow tắt thì dừng theo dõi.
	vpH := height - 1
	if vpH < 1 {
		vpH = 1
	}
...
func renderCoCreatePromptPanel(width, height int, state *cocreateState) string {
	readyLabel := "Sẵn sàng bắt đầu sáng tác"
	if state.stage {
	...
	emptyHint := "AI sẽ liên tục tổng hợp ở đây một lệnh cuối cùng có thể đi thẳng vào sáng tác."
	panelTitle := ":: Lệnh sáng tác hiện tại"
	if state.stage {
		emptyHint = "AI sẽ liên tục tổng hợp ở đây brief hướng đi của giai đoạn tiếp theo."
	...
	if state.promptVP.TotalLineCount() > state.promptVP.VisibleLineCount() {
		...
		case state.promptVP.AtTop():
			hint = "↓ Còn nội dung bên dưới, cuộn con lăn hoặc PgDn để xem"
		case state.promptVP.AtBottom():
			hint = "↑ Còn nội dung bên trên, cuộn con lăn hoặc PgUp để xem"
		default:

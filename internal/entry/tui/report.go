...
type reportState struct {
	reqID      int
	report     *diag.Report
	exportPath string // đường dẫn file chẩn đoán đã ẩn thông tin nhạy cảm, render ở đầu báo cáo để dán issue
	loading    bool
	renderW    int
	startedAt  time.Time
...
func renderReportText(report diag.Report, width int, exportPath string, startedAt, finishedAt time.Time) string {
	...
	// Chẩn đoán đã ẩn thông tin nhạy cảm và xuất ra → hướng dẫn người dùng dán issue
	if exportPath != "" {
		exportStyle := lipgloss.NewStyle().Foreground(colorAccent2)
		b.WriteString(exportStyle.Render("Đã xuất chẩn đoán ẩn thông tin nhạy cảm (có thể dán vào GitHub issue)"))
		b.WriteString("\n")
	...
	// Dòng 2: đánh giá + viết lại + điểm trung bình
...
func renderReportLoadingText(width int, startedAt time.Time) string {
	...
	b.WriteString("\n\n")
	b.WriteString(bodyStyle.Render(wrapText("Đang đọc sản phẩm output của tiểu thuyết hiện tại và phân tích các vấn đề về luồng, chất lượng, quy hoạch và ngữ cảnh. Có thể mất vài giây với dự án lớn.", width)))
	b.WriteString("\n\n")
	b.WriteString(hintStyle.Render("Esc có thể đóng panel trước, lần sau mở lại sẽ tạo mới sau khi phân tích nền hoàn tất."))
	return b.String()
}

...
func (h *terminalAskUser) askOne(ctx context.Context, q tools.Question) (string, string, error) {
	...
	fmt.Fprintln(h.out, "  0. Tự nhập nội dung")
	...
	for {
		...
		if q.MultiSelect {
			fmt.Fprint(h.out, "Nhập số thứ tự, nhiều lựa chọn cách nhau bởi dấu phẩy: ")
		} else {
		...
		if line == "" {
			fmt.Fprintln(h.out, "Đầu vào không được để trống, hãy thử lại.")
			continue
		}
		if line == "0" {
			...
			if note == "" {
				fmt.Fprintln(h.out, "Nội dung tùy chỉnh không được để trống, hãy thử lại.")
				continue
			}
		...
		if err != nil {
			fmt.Fprintf(h.out, "%v, hãy thử lại.\n", err)
			continue
		}
...
func parseSelections(line string, options []tools.Option, multi bool) ([]string, error) {
	...
	if !multi && len(parts) > 1 {
		return nil, fmt.Errorf("câu hỏi này chỉ cho phép chọn một")
	}
	...
	if len(labels) == 0 {
		return nil, fmt.Errorf("hãy chọn ít nhất một lựa chọn")
	}
	return labels, nil
}

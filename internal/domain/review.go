...
// DimensionScore điểm thẩm định theo từng chiều.
type DimensionScore struct {
	...
	Comment   string `json:"comment,omitempty"` // kết luận tóm tắt cho chiều này
}

// ReviewEntry mục thẩm định của Editor.
type ReviewEntry struct {
	...
	ContractNotes    string             `json:"contract_notes,omitempty"`  // mô tả tóm tắt về việc thực hiện contract
	Verdict          string             `json:"verdict"`                   // accept / polish / rewrite
	Summary          string             `json:"summary"`
	AffectedChapters []int              `json:"affected_chapters,omitempty"` // số chương cần viết lại/trau chuốt
}

// CriticalCount trả về số lượng vấn đề cấp độ critical.
...
// ErrorCount trả về số lượng vấn đề cấp độ error.

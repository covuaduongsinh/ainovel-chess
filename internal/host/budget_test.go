...
func TestBudgetSentinelWarnOnceThenBoundaryStop(t *testing.T) {
	...
	// Chua dat nguong: khong co tac dung phu
	s.OnCost(5)
	if len(r.reports) != 0 {
		t.Fatalf("below warn ratio should be silent, got %v", r.reports)
	}

	// Vuot nguong canh bao: chinh xac mot lan warn, callback lap lai khong phat them
	s.OnCost(8.5)
	s.OnCost(9)
	...
	// Vuot gioi han: vao stopPending, phat error, nhung khong dung ngay (mac dinh doi ranh gioi)
	s.OnCost(10.5)
	...
	// Su kien khong phai ranh gioi khong kich hoat
	...
	// Ranh gioi subagent: chinh xac mot lan dung may, ranh gioi lap lai khong dung them
	r.cost = 10.5
	if !s.HandleBoundary() {
...
func TestBudgetSentinelJumpStraightPastLimit(t *testing.T) {
	...
	// Mot callback nhay thang qua canh bao va gioi han: warn va error moi cai chinh xac mot lan
	s.OnCost(12)
	if len(r.reports) != 2 {
...
func TestBudgetSentinelHardStop(t *testing.T) {
	...
	// Ranh gioi tiep theo khong dung them lan nua
	r.cost = 11
	s.HandleEvent(subagentEndEvent())
	if len(r.aborts) != 1 {
...
func TestBudgetSentinelRefuse(t *testing.T) {
	...
	r.cost = 10 // Chinh xac bang gioi han -> tu choi
	if err := s.Refuse(); err == nil {
...
func TestBudgetSentinelZeroCostBlindWarning(t *testing.T) {
	...
	// Ghi tai lien tuc gia tri khong: den blindZeroStreak bút chinh xac mot lan canh bao vung mu, sau do im lang
	...
	// Mo hinh tinh gia binh thuong khong nen bao nham: tong tich luy tang dan moi lan ghi tai
	r2 := &budgetRecorder{}
...
func TestBudgetSentinelBlindWarningAfterModelSwitch(t *testing.T) {
	// Giua chung truoc khi /model chuyen sang mo hinh khong gia: total dung o gia tri lich su khac 0 nhung khong tang nua, van can canh bao
	r := &budgetRecorder{}
	s := r.sentinel(bootstrap.BudgetConfig{BookUSD: 100, WarnRatio: 0.8})

	for i := range 5 {
		s.OnCost(1.0 * float64(i+1)) // Giai doan tinh gia: tong tang den $5
	}
	for range blindZeroStreak {
		s.OnCost(5.0) // Chuyen sang mo hinh khong gia: tong bi ghim
	}

...
func buildWriterStoreSummaryText(s *store.Store, budgetTokens int) (string, bool, error) {
	...
	return "Nội dung dưới đây lấy từ store lưu trữ bền vững của tiểu thuyết, dùng để khôi phục ngữ cảnh viết sau khi nén.\n\n" + strings.Join(parts, "\n\n"), true, nil
}
...
func writerStoreSummarySections(state *writerStoreSummaryState) []writerStoreSection {
	return []writerStoreSection{
		...
		{heading: "Snapshot nhân vật", data: state.snapshots},
		{heading: "Phục bút đang hoạt động", data: state.foreshadow},
		{heading: "Vấn đề thẩm định chờ sửa", data: state.pendingReviews},

...
- **Tính năng tương đương TUI**: Xem trực tiếp luồng sáng tác, can thiệp người dùng (steer), tạm dừng / tiếp tục, xem tiến độ / dàn ý / nhân vật, xuất Markdown/TXT/EPUB (mặc định cả 3), chuyển đổi mô hình và cường độ suy luận, cùng tạo quy hoạch, nhập tiếp nối, hồ sơ hành văn, chẩn đoán `/diag`.
...
Nhập `/export` trong TUI có thể hợp nhất và xuất các chương đã hoàn thành. **Mặc định xuất cùng lúc 3 định dạng** — Markdown, TXT và EPUB — vào `{novelDir}/{NovelName}.{md,txt,epub}`. Xuất là thao tác chỉ đọc, có thể lấy "sản phẩm giai đoạn hiện tại" bất kỳ lúc nào giữa chừng viết, không ảnh hưởng đến Coordinator đang chạy.

Đường dẫn giờ được hiểu là **đường dẫn base** (không có đuôi định dạng):

- **Để trống / không kèm đuôi** → xuất cả **3 file** `.md` + `.txt` + `.epub`.
- **Kèm đuôi nhận biết** `.md` / `.txt` / `.epub` → chỉ xuất **đúng 1 định dạng** đó.

```text
/export                            # 3 file .md/.txt/.epub tại {novelDir}/{NovelName}.*
/export ~/anh-sang                 # base không đuôi → 3 file ~/anh-sang.md/.txt/.epub
/export ~/anh-sang.md              # đuôi .md  → chỉ Markdown
/export ~/anh-sang.txt             # đuôi .txt → chỉ TXT
/export ~/anh-sang.epub            # đuôi .epub → chỉ EPUB (Apple Books / WeChat Reading / Kindle converter đọc được)
/export from=10 to=30 --overwrite  # Khoảng chương + ghi đè (vẫn ra 3 file)
/export from=10 ~/x.epub --overwrite
```

- **MD (Markdown)** — Tiêu đề ATX: tên sách `# Tên sách`, đầu tập `## Tập N — Tiêu đề` (chỉ khi dàn ý phân tầng), mỗi chương `## Chương N — Tiêu đề`; thân chương giữ nguyên văn. Dễ đọc, dễ mở trong bất kỳ trình soạn thảo nào và dễ convert tiếp (pandoc → DOCX/PDF…).
...
Các chương chưa hoàn thành trong khoảng sẽ bị bỏ qua và hiển thị trong kết quả, không tính là lỗi. Chi tiết đầy đủ (quy tắc đường dẫn, hợp đồng API Web): [docs/export.md](docs/export.md).
